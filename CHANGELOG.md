...
### Fixed
- `ReconnectError` now subclasses `ConnectionError` instead of bare `Exception`, so callers that catch `ConnectionError` (the standard Python pattern for network failures) also catch reconnect exhaustion.
